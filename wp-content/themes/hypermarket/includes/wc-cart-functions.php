...
function wc_protected_product_add_to_cart( $passed, $product_id ) {
	if ( post_password_required( $product_id ) ) {
		$passed = false;
		wc_add_notice( __( 'Produk ini dilindungi dan tidak dapat dibeli.', 'woocommerce' ), 'error' );
	}
	return $passed;
}
...
function wc_add_to_cart_message( $products, $show_qty = false, $return = false ) {
	...
	/* translators: %s: product name */
	$added_text = sprintf( _n( '%s telah ditambahkan ke keranjang Anda.', '%s telah ditambahkan ke keranjang Anda.', $count, 'woocommerce' ), wc_format_list_of_items( $titles ) );

	// Output success messages.
	if ( 'yes' === get_option( 'woocommerce_cart_redirect_after_add' ) ) {
		$return_to = apply_filters( 'woocommerce_continue_shopping_redirect', wc_get_raw_referer() ? wp_validate_redirect( wc_get_raw_referer(), false ) : wc_get_page_permalink( 'shop' ) );
		$message   = sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', esc_url( $return_to ), esc_html__( 'Lanjut Berbelanja', 'woocommerce' ), esc_html( $added_text ) );
	} else {
		$message = sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', esc_url( wc_get_page_permalink( 'cart' ) ), esc_html__( 'Lihat Keranjang', 'woocommerce' ), esc_html( $added_text ) );
	}
	...
	foreach ( $packages as $i => $package ) {
		...
		wc_get_template(
			'cart/cart-shipping.php', array(
				...
				'package_name'             => apply_filters( 'woocommerce_shipping_package_name', ( ( $i + 1 ) > 1 ) ? sprintf( _x( 'Pengiriman %d', 'paket pengiriman', 'woocommerce' ), ( $i + 1 ) ) : _x( 'Pengiriman', 'paket pengiriman', 'woocommerce' ), $i, $package ),
				'index'                    => $i,
...
function wc_cart_totals_coupon_label( $coupon, $echo = true ) {
	...
	$label = apply_filters( 'woocommerce_cart_totals_coupon_label', sprintf( esc_html__( 'Kupon: %s', 'woocommerce' ), $coupon->get_code() ), $coupon );
...
function wc_cart_totals_coupon_html( $coupon ) {
	...
	if ( $coupon->get_free_shipping() && empty( $amount ) ) {
		$discount_amount_html = __( 'Kupon pengiriman gratis', 'woocommerce' );
	}
...
function wc_cart_totals_order_total_html() {
	...
	if ( wc_tax_enabled() && WC()->cart->display_prices_including_tax() ) {
		...
		if ( ! empty( $tax_string_array ) ) {
			...
			$estimated_text = WC()->customer->is_customer_outside_base() && ! WC()->customer->has_calculated_shipping() ? sprintf( ' ' . __( 'estimasi untuk %s', 'woocommerce' ), WC()->countries->estimated_for_prefix( $taxable_address[0] ) . WC()->countries->countries[ $taxable_address[0] ] ) : '';
