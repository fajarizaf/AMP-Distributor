<form action="" method="post" class="pok-custom-form">
	...
	if ( function_exists( 'ini_get' ) ) {
		...
		$max_row = floor( ( $max_input_vars / 9 ) - 2 );
	}
	?>
	...
	<div class="pok-custom-cost">
		<table class="wc_shipping widefat wp-list-table custom-costs striped">
			<thead>
				<tr>
					<th colspan="3" width="50%"><?php esc_html_e( 'Destination', 'pok' ); ?></th>
					<th width="10%"><?php esc_html_e( 'Courier', 'pok' ); ?></th>
					<th width="15%"><?php esc_html_e( 'Service Name', 'pok' ); ?></th>
					<th width="15%"><?php esc_html_e( 'Cost per Kg', 'pok' ); ?></th>
					...
					foreach ( $costs as $key => $cost ) {
						...
						<tr>
							...
							<td>
								...
								<?php echo esc_html( 'custom' === $cost['courier'] ? $settings['custom_cost_courier'] : $couriers[ $cost['courier'] ] ); ?>
							</td>
							<td>
					...
					<td>
						<select id="select_courier">
							...
							<?php if ( ! empty( $couriers ) ) : ?>
								<?php foreach ( $couriers as $k => $c ) : ?>
									<option value="<?php echo esc_attr( $k ); ?>"><?php echo esc_html( $c ); ?></option>
								<?php endforeach; ?>
							<?php endif; ?>
						</select>
					</td>
