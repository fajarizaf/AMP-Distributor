jQuery(function($) {
	...
	$('#set-rajaongkir-key').on('click', function() {
		...
		if ( "" === api_key ) {
			$('.rajaongkir-key-response').text( pok_translations.rajaongkir_key_empty );
			return;
		}
	...
	$('.pok-checker').on( 'change', '#select_province', function() {
		...
		pok_load_city_list( province_id, 'checker' );
	} );
	$('.pok-checker').on( 'change', '#select_city', function() {
		var city_id = $(this).val();
		pok_load_district_list( city_id, 'checker' );
	});
	...
	$('#select_province, #select_city').trigger('change');
	...
	$('.pok-custom-cost').on( 'click', '#add-cost', function() {
		...
		if ( 'custom' === courier ) {
			...
			var courier_text = $('#ekspedisi').find(":selected").text();;
		}		
