		...
		class GazChap_WC_PurchaseOrder_Gateway extends WC_Payment_Gateway {
			...
			public function init_form_fields() {
				$this->form_fields = apply_filters( 'wc_offline_form_fields', array(
					...
					'ask_po_number' => array(
						...
						'type'    => 'checkbox',
						'label'   => __( 'Tanyakan kepada pelanggan untuk nomor pesanan pembelian saat checkout', 'gazchaps-woocommerce-purchase-order-payment-gateway' ),
						'default' => 'yes'
					),
					'require_po_number' => array(
						'title'   => __( 'Require PO Number', 'gazchaps-woocommerce-purchase-order-payment-gateway' ),
						'type'    => 'checkbox',
						'label'   => __( 'Wajibkan nomor pesanan pembelian untuk dimasukkan saat pembayaran', 'gazchaps-woocommerce-purchase-order-payment-gateway' ),
						'default' => 'yes'
					),
					'add_po_number_to_email' => array(
						'title'   => __( 'Add PO Number to Emails', 'gazchaps-woocommerce-purchase-order-payment-gateway' ),
						'type'    => 'checkbox',
						'label'   => __( 'Menambahkan nomor pesanan pembelian yang disediakan ke email konfirmasi pesanan', 'gazchaps-woocommerce-purchase-order-payment-gateway' ),
						'default' => 'yes'
					),
					'ask_address' => array(
						'title'   => __( 'Ask for Address', 'gazchaps-woocommerce-purchase-order-payment-gateway' ),
						'type'    => 'checkbox',
						'label'   => __( 'Mintalah pelanggan untuk alamat untuk mengirim faktur ke', 'gazchaps-woocommerce-purchase-order-payment-gateway' ),
						'default' => 'no'
					),
					'add_address_to_email' => array(
						'title'   => __( 'Add Address to Emails', 'gazchaps-woocommerce-purchase-order-payment-gateway' ),
						'type'    => 'checkbox',
						'label'   => __( 'Menambahkan alamat yang disediakan ke email konfirmasi pesanan', 'gazchaps-woocommerce-purchase-order-payment-gateway' ),
						'default' => 'no'
					),
					'use_billing_address' => array(
						'title'   => __( 'Use Billing Address', 'gazchaps-woocommerce-purchase-order-payment-gateway' ),
						'type'    => 'checkbox',
						'label'   => __( 'Pra-isi Alamat Faktur dengan alamat penagihan pelanggan, jika mereka masuk', 'gazchaps-woocommerce-purchase-order-payment-gateway' ),
						'default' => 'no'
					),
				) );
			}
			...
			<?php if ( $this->ask_po_number ): ?>
				<p class="form-row form-row-wide">
					<label for="gc-wc-popg-number"><?php esc_html_e('Silahkan masukan nomor Purchase Order anda', 'gazchaps-woocommerce-purchase-order-payment-gateway'); ?><?php if ( $this->require_po_number ): ?> <span class="required">*</span><?php endif; ?></label>
					<input type="text" id="gc-wc-popg-number" name="gazchap_purchase_order[number]" class="input-text">
				</p>
			<?php endif; ?>

			<?php if ( $this->ask_address ): ?>
				<p class="form-row form-row-wide"><strong><?php esc_html_e('Dimana alamat anda untuk kami kirimkan faktur?', 'gazchaps-woocommerce-purchase-order-payment-gateway'); ?></strong></p>
			...
			public function validate_fields() {
				...
				if ( empty( $_POST['gazchap_purchase_order'] ) ) {
					...
					if ( $this->ask_po_number && $this->require_po_number && empty( $data['number'] ) ) {
						wc_add_notice( __( 'Mohon untuk memasukan nomor Purchase Order anda.', 'gazchaps-woocommerce-purchase-order-payment-gateway' ), 'error' );
						$valid = false;
					}

					if ( $this->ask_address ) {
						if ( empty( $data['contact'] ) ) {
							wc_add_notice( __( 'Harap masukkan Nama Kontak untuk alamat faktur.', 'gazchaps-woocommerce-purchase-order-payment-gateway' ), 'error' );
							$valid = false;
						}

						if ( empty( $data['company'] ) ) {
							wc_add_notice( __( 'Harap masukkan Perusahaan / Organisasi untuk alamat faktur.', 'gazchaps-woocommerce-purchase-order-payment-gateway' ), 'error' );
							$valid = false;
						}

						if ( empty( $data['address1'] ) ) {
							wc_add_notice( __( 'Harap masukkan Alamat (baris 1) untuk alamat faktur.', 'gazchaps-woocommerce-purchase-order-payment-gateway' ), 'error' );
							$valid = false;
						}

						if ( empty( $data['city'] ) ) {
							wc_add_notice( __( 'Silakan masukkan Kota untuk alamat faktur.', 'gazchaps-woocommerce-purchase-order-payment-gateway' ), 'error' );
							$valid = false;
						}

						if ( empty( $data['postcode'] ) ) {
							wc_add_notice( __( 'Silakan masukkan Kode Pos untuk alamat faktur.', 'gazchaps-woocommerce-purchase-order-payment-gateway' ), 'error' );
							$valid = false;
						}
					}
				}
