...
wc_bof_handler.prototype.single_add_to_cart_handler = function ( formid, clickelem ) {
	...
	jQuery.ajax( {
		...
		if( typeof(res.data) != "undefined" && res.data.indexOf('error') != -1){
			var scroll_y = $( "article" ).length ? $("article").offset().top : $(".backEndResponse").offset().top - 10;
			$("html, body").animate({ scrollTop: scroll_y }, "fast");
		}
		...
		wcbofh.form.find( 'table.wcbulkorderproducttbl tr[data-rowcount=' + rowCount + ']' ).each( function () {
