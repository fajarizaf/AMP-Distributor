...
class WooCommerce_Bulk_Order_Form_Standard_Product_Search extends WooCommerce_Bulk_Order_Form_Template_Product_Search {
	...
	public function extract_products( $arr ) {
		...
		foreach ( $this->pid_result as $pid ) {
			...
			if ( get_post_meta( $pid, '_visibility', true ) == 'hidden' ) {
				continue;
			}
			...
			if ( ( 'product' == $post_type ) && empty( $children ) ) {
				...
				$product = new WC_Product_Variation( $pid );
				...
				foreach ( $attributes as $name => $value ) {
					...
					$terms = get_the_terms( $id, $name );
					if ( ! empty( $terms ) ) {
						foreach ( $terms as $term ) {
							if ( is_object( $term ) && isset( $term->name ) && strtolower( $term->name ) == $value ) {
								$value = $term->name;
							}
						}
					}
					...
					if ( 'attributes' === $arr['product_attributes'] ) {
						$title .= ' ' . $attr_value . ' ';
					} else {
			...
			$suggestion               = array();
			$suggestion['label']      = html_entity_decode( apply_filters( 'wc_bulk_order_form_label', $label, $price, $title, $sku, $active_currency ) );
